...
contract("ZSC", async (accounts) => {
  ...
  it("should allow transferring (6 decoys and miner)", async () => {
    ...
    await alice.transfer(
      "Bob",
      10,
      ["Carol", "Dave", "Bob1", "Carol1", "Dave1", "Miner1"],
      "Miner"
    );
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await bob.updateBalance();
    assert.equal(bob.account.balance(), 30, "Transfer failed");
    const fee = await zsc.fee.call();
    assert.equal(miner.account.balance(), fee, "Fees failed");
  });
  ...
  it("should allow transferring without decoys or miner", async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await zuza.updateBalance();
    assert.equal(zuza.account.balance(), 5, "Transfer failed");
  });

  it("should allow transferring without decoys but with miner", async () => {
    await alice.transfer("Carol", 5, [], "Miner");
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await carol.updateBalance();
    assert.equal(carol.account.balance(), 5, "Transfer failed");
  });
});
